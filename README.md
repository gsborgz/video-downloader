...
- O YouTube às vezes bloqueia requisições vindas de IPs de datacenter (é o caso das funções serverless da Vercel) com a mensagem "Sign in to confirm you're not a bot". Isso é um comportamento do lado do YouTube, fora do nosso controle. Como mitigação, `lib/yt-dlp.ts` primeiro tenta a extração normal (melhor qualidade) e, especificamente quando bate nesse erro, tenta de novo forçando clients alternativos do YouTube (`android`/`tv`/`ios`, via `--extractor-args`) que geralmente não passam por essa checagem — ao custo de formatos mais limitados (às vezes só 360p) nesse cenário de fallback. Não é uma garantia: o YouTube muda essas regras com frequência, e não há solução 100% confiável sem autenticação (cookies de uma conta logada).
